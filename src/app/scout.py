...
NOTE on race lengths: mapping a CSR delta to an actual handicapped race length
needs the NAPA race chart, which we don't have yet. `race_length` is a pluggable
placeholder; the CSR edge per game is the load-bearing signal until the chart
is supplied (and a probability model is Phase 6).
"""
...
def race_length(my_csr: int, opp_csr: int, game: int) -> int | None:
    """Placeholder: real handicapped race lengths require the NAPA race chart.

    Returns None until the chart is supplied. Kept as a seam so the grid/cell
    code already calls through to it.
    """
    return None
...
def render_cell(cell: Cell) -> str:
    """Drill-down: the three per-game edges + the LC picks for one pairing."""
    lines = [
        f"{cell.my_player} ({cell.my_id})  vs  {cell.opp_player} ({cell.opp_id})",
        f"  {'game':>5} {'mine':>5} {'opp':>5} {'edge':>6}",
    ]
    for e in cell.edges:
        flag = "  <- I'd pick" if e is cell.my_pick else (
            "  <- they'd pick" if e is cell.opp_pick else "")
        lines.append(f"  {e.game:>5} {e.my_csr:>5} {e.opp_csr:>5} {e.edge:>+6}{flag}")
    lines.append(
